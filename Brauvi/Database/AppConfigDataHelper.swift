...
class AppConfigDataHelper: DataHelperProtocol {
    ...
    static func createTable() {
        do {
            ...
            self.log.info( insert(item: AppConfig(id: 0, key: databaseSchemaVersionKey, value: "1")) )
        } catch {
    ...
    static func update(item: T) {
        do {
            ...
            try DB.run(query.update(key <- item.key, value <- item.value))
        } catch {
            log.error(error)
        }
    }
