...
class WebUploader: NSObject {
    ...
    var multimediaArray: Array<VideoModel>!
    
    init(uploadDirectory: String, multimediaArray: Array<VideoModel>) {
        super.init()
    ...
    func start() -> String {
        ...
        self.webUploader.addHandler(forMethod: "UPDATE", path: "/reloadData", request: GCDWebServerMultiPartFormRequest.self,processBlock: { (request: GCDWebServerRequest?) -> GCDWebServerResponse? in
            self.log.info("VideoModelManager.shared.reloadData()")
            VideoModelManager.shared.reloadData()
            
            self.multimediaArray = VideoModelManager.shared.data
        ...
        do {
            ...
            if self.webUploader.serverURL != nil {
                address = self.webUploader.serverURL.absoluteString
            }
